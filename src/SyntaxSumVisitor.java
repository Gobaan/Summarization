...
public class SyntaxSumVisitor extends SimpleFileVisitor<Path> { 
	...
	public static void main(String[] args) {		
		...
		try {
			...
			Files.walkFileTree(startingDir, new SyntaxSumVisitor());
	...
	public FileVisitResult postVisitDirectory(Path dir, IOException exc)
            throws IOException
        {
            ...
            try {
				...
				int docnum = 0;
				for (Document doc : this.docs.documents) {
					docnum += 1;
					for (Sentence s : doc.sentences) {
						for (String term : s.terms) {
							Integer index = vocabIndices.get(term);
							if (index != null) {
								ws.add(index);
								ds.add(docnum);
							}
